SELECT 
    ...
    SUM(
        CASE 
            WHEN hospitalization_cost.hosp_days > hospitalization_cost.mean_los_days 
            THEN hospitalization_cost.base_cost 
...
FROM (
    ...
    JOIN ken_code k 
        ON k.code = h.ken_code
) AS hospitalization_cost
-- Only patients who go hospitalized 3 times in same department
WHERE hospitalization_cost.patient_amka IN (
    SELECT patient_amka
    FROM hospitalization
    GROUP BY patient_amka, department_id
